...
async function resolvePartnerUserId(admin: ReturnType<typeof createClient>, partnerRef: string, sub: string): Promise<string | null> {
  ...
  if (error) {
    console.error("thread-activity partner listUsers error:", error.message);
    return null;
  }
...
async function fetchRest(url: string, serviceRoleKey: string) {
  const res = await fetch(url, {
    headers: { apikey: serviceRoleKey, Authorization: `Bearer ${serviceRoleKey}` },
  });
  if (!res.ok) {
    const errText = (await res.text()).slice(0, 300);
    console.error(`thread-activity rest ${res.status}: ${errText}`);
    return null;
  }
  return await res.json();
}

function truncateScanMessages(scans: unknown): unknown {
  if (!Array.isArray(scans)) return [];
  return scans.map((row: Record<string, unknown>) => {
    const messages = Array.isArray(row.messages) ? (row.messages as unknown[]) : [];
    const truncated = messages.slice(-5);
    return { ...row, messages: truncated, messages_truncated: messages.length > truncated.length };
  });
}
...
serve(async (req) => {
  ...
  if (req.method === "GET" && new URL(req.url).searchParams.get("health") === "1") {
    return jsonResponse({ ok: true, function: "thread-activity" });
  }
  if (req.method !== "GET") {
  ...
  const threadSelect = [
    ...
    "snapshot_unread_count",
    "snapshot_body",
    "snapshot_from_me",
    "snapshot_msg_type",
    "snapshot_ack",
    "snapshot_msg_timestamp",
    "snapshot_has_reaction",
    "snapshot_is_forwarded",
    "last_scan",
    "scan_captured_at",
    "scan_message_count",
  ].join(",");
  ...
  threadParams.set("user_id", `eq.${userId}`);
  threadParams.set("select", threadSelect);
  threadParams.set("order", "snapshot_captured_at.desc.nullslast");
  threadParams.set("limit", String(limit));
  if (threadId) threadParams.set("thread_id", `eq.${threadId}`);

  const snapParams = new URLSearchParams();
  snapParams.set("user_id", `eq.${userId}`);
  snapParams.set("select", "*");
  snapParams.set("order", "captured_at.desc");
  snapParams.set("limit", String(limit));
  if (threadId) snapParams.set("thread_id", `eq.${threadId}`);

  const scanParams = new URLSearchParams();
  scanParams.set("user_id", `eq.${userId}`);
  scanParams.set("select", "id,provider,thread_id,captured_at,message_count,messages,source,extension_version,created_at");
  scanParams.set("order", "captured_at.desc");
  scanParams.set("limit", String(limit));
  if (threadId) scanParams.set("thread_id", `eq.${threadId}`);

  try {
    const [threads, snapshots, scans] = await Promise.all([
      fetchRest(`${SUPABASE_URL}/rest/v1/thread_states?${threadParams.toString()}`, SUPABASE_SERVICE_ROLE_KEY),
      fetchRest(`${SUPABASE_URL}/rest/v1/chat_snapshots?${snapParams.toString()}`, SUPABASE_SERVICE_ROLE_KEY),
      fetchRest(`${SUPABASE_URL}/rest/v1/chat_scans?${scanParams.toString()}`, SUPABASE_SERVICE_ROLE_KEY),
    ]);
    ...
    return jsonResponse({
      ...
      scans: truncateScanMessages(scans),
    });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error(`thread-activity ERROR user=${userId}: ${msg}`);
