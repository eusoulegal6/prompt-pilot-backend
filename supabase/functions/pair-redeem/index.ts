...
serve(async (req) => {
  ...
  // The pairing code was created by an authenticated dashboard session
  // (pair-create binds it to auth.uid()), so codeRow.user_id IS the dashboard
  // user. The extension itself has no session at redeem time, so we trust the
  // one-time, short-lived code as the binding mechanism.
  const dashboardUserId = codeRow.user_id;
