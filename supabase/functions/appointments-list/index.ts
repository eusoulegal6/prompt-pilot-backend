...
async function resolvePartnerUserId(
  ...
  if (createErr && !msg.includes("already") && !msg.includes("registered") && !msg.includes("exists")) {
    console.error("appointments-list partner createUser error:", createErr.message);
    return null;
  }
  ...
  if (listErr) {
    console.error("appointments-list partner listUsers error:", listErr.message);
    return null;
  }
...
serve(async (req) => {
  ...
  if (req.method === "GET" && new URL(req.url).searchParams.get("health") === "1") {
    return jsonResponse({ ok: true, function: "appointments-list" });
  }
  ...
  if (error) {
    console.error("appointments-list query error:", error.message);
    return jsonResponse({ error: "Failed to read appointments." }, 500);
  }
